...
This deployment is preloaded with synthetic documents (the fictional "Maya
Patel" profile — see `sample_documents/`). **Uploading is temporarily
enabled for hackathon judging**, so judges can exercise the real upload →
processing → ready workflow themselves; **deletion stays disabled** at all
times (see `docs/DEPLOYMENT.md`'s "Public-demo read-only mode" section for
the toggles and exact mechanism). Everything else — document analysis, the
O-1A planner, grounded chat, and Official Updates — works normally against
the preloaded documents.
**Only upload synthetic test documents — never a real person's real
immigration documents or personal information** — this build has no
authentication and no per-user data isolation, so anything uploaded is
visible to every other visitor of this deployment.
